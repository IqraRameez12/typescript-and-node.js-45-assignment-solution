...
let current_users=["ali","hamza","atif","Aqib","Haadi","hammad"];

let new_users=["asif","adil","aqib","haadi","hussnain","abdul"];

//we will use forEach method:


new_users.forEach(new_users=>{
    let newUserLower=new_users.toLocaleLowerCase()

 //we will use some() method:
  
 let userNameTaken=current_users.some(current_users=> current_users.toLowerCase()==newUserLower)
 if(userNameTaken){
    console.log(`\n${new_users} need to choose a new username because it's already taken\n`);
